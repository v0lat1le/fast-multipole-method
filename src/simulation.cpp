...
Cells build_quadtree(std::span<const Vec2d> points, int max_levels, int max_points) {
    ...
    for (int level = 0; level < max_levels-1 && !cells.levels[level].empty(); ++level) {
        cells.levels.push_back({});
        for (const auto& cell: cells.levels[level]) {
...
void compute_acceleration_multipoles(const Cells& cells, std::span<const Vec2d> positions, std::span<const double> masses, std::span<Vec2d> accelerations) {
    ...
    for (std::size_t level=1; level<multipoles.levels.size(); ++level) {  // skipping level 0 to avoid buncha level==0 checks
        for (auto& multipole: multipoles.levels[level]) {
            const auto has_children = multipoles.has_children(level, multipole.first);
            if (not has_children) {
            ...
            for (auto parent_neighbours_coords: multipoles.neighbours_coords(level-1, parent_coords)) {
                ...
                if (not multipoles.is_adjacent(level, multipole.first, parent_level, parent_neighbours_coords)) {
                    ...
                    if (parent_neighbour == cells.levels[parent_level].end()) {
                        auto [found_level, found_cell] = cells.find_childless_parent(parent_level, parent_neighbours_coords);
                        if (found_level == 0) {
                            continue;
                        }
                    ...
                    bool parent_neighbour_has_children = false;
                    for (auto& cousin_coords: multipoles.children_coords(level-1, parent_neighbours_coords)) {
                        auto cousin = cells.levels[level].find(cousin_coords);
                        if (cousin == cells.levels[level].end()) {
                            continue;
                        }
                        parent_neighbour_has_children = true;
                    ...
                    if (parent_neighbour_has_children or has_children) {
                        continue;
                    }
                    auto parent_neighbour = cells.levels[level-1].find(parent_neighbours_coords);
                    if (parent_neighbour == cells.levels[level-1].end()) {
                        auto [found_level, found_cell] = cells.find_childless_parent(level-1, parent_neighbours_coords);
                        if (found_level == 0) {
                            continue;
                        }
                        skip_list[skip_list_count++] = std::make_pair(found_level, found_cell->first);
